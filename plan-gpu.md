...
Completed locally on 2026-07-10 through commit `d444c81`.

- `169c508 refactor: extract frohlich physics scalars`
- `20874c9 feat: add flat frohlich diagram`
- `c18e3d1 feat: add flat frohlich updates`
- `f7982c7 feat: validate flat frohlich measurement`
- `d444c81 test: add flat frohlich ratio parity`

Implemented:

- `crates/apps/rmc-frohlich/src/physics.rs` holds scalar `[f64; 3]` physics leaves used by
  the slotmap and flat update paths.
- `FlatDiagram` lives in `crates/apps/rmc-frohlich/src/flat/mod.rs`, with fixed
  `2 * max_order_gpu + 2` capacity, `u32::MAX` nulls, dense `storage`/`storage_idx`, and
  storage-order-preserving conversion from `Diagram`.
- `RunConfig::max_order_gpu` was added with serde default `256`.
- All eight flat updates live in `flat/updates.rs`, consume RNG draws in slotmap order,
  and return `< 0.0` for flat capacity overflow.
- `PolaronMeasurement` supports `FlatDiagram` through `new_flat`; flat runs reuse the
  existing `PolaronStats`/jackknife path.

Validation currently present:

- `tests/flat_ratio_parity.rs`: proptest over valid states generated by the slotmap
  engine, comparing every update's attempt probability against the flat path.
- `tests/flat_diagram.rs`: 100,000-step slotmap-vs-flat lockstep check with identical
  update index, accept/reject, impossible flags, and state parity within fp tolerance.
- `tests/flat_diagram.rs`: short `run_chain` slotmap-vs-flat estimator parity through
  `PolaronMeasurement`.

Latest verification at this point:
...
Result: `42 passed` for `rmc-frohlich`, `183 passed` workspace after the Phase 2
batched-driver commit below.

Outstanding Phase 1 user checkpoint:

```bash
rtk cargo bench-compare -p rmc-frohlich --bin rmc-frohlich --reps 5 --metric-regex 'steps/sec:\s*([\d.]+)' -- bench fixtures/bench-frohlich.json
```
